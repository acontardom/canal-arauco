// scripts/regenerar-pdfs-edp7.js
//
// Regenera los PDFs firmados de los protocolos HA del EDP7 que se subieron a
// Storage con "Sin camiones registrados", por el [] hardcodeado que recibía
// construirDocumentoPDF en confirmarFirma() (Firma.jsx). Corregido en 99cb71c.
//
// PRESERVA la firma original: lee firma_imagen_url y firma_fecha de cada fila,
// reincrusta esa misma imagen y esa misma fecha en el PDF, y NO las reescribe.
// El UPDATE toca únicamente pdf_firmado_url y fecha_modificacion.
//
// Diferencias respecto a regenerar-pdfs-edp6.js:
//   - Los protocolos se consultan (edp = '7' + protocolo_id HA), no hardcodeados
//   - Modo --dry-run: lista lo que haría sin escribir nada
//   - Si tras la consulta un protocolo sigue sin camiones, se OMITE en vez de
//     volver a subir un PDF vacío encima del existente
//
// Ejecución (desde la raíz del proyecto):
//   node scripts/regenerar-pdfs-edp7.js --dry-run   # inspeccionar
//   node scripts/regenerar-pdfs-edp7.js             # aplicar
//
// Requiere: Node.js 18+
...
// ─── Respaldo del PDF vigente ────────────────────────────────────────────────

const BACKUP_DIR = join(ROOT, 'backup-pdfs-edp7');

// Nombre legible: caida-25-HA_RADIER.pdf (no el UUID).
function nombreRespaldo(row) {
  const limpio = v => String(v ?? 'sd').replace(/[^a-zA-Z0-9._-]+/g, '_');
  return `${limpio(row.tipo)}-${limpio(row.entidad_id)}-${limpio(row.protocolo_id)}.pdf`;
}

// Descarga el PDF vigente y lo guarda en BACKUP_DIR.
// Devuelve la ruta local, o null si algo falla (el llamador debe omitir el protocolo).
async function respaldarPDFVigente(row, pdfPath) {
  ...
  try {
    ...
    const destino = join(BACKUP_DIR, nombreRespaldo(row));
...
// ─── Selección de protocolos (EDP7 — HA) ─────────────────────────────────────

const EDP_OBJETIVO = '7';
const TIPOS_HA     = ['HA_RADIER', 'HA_MURO'];
const ESPERADOS    = 15;
const DRY_RUN      = process.argv.includes('--dry-run');

async function obtenerProtocolosEDP7() {
  const { data, error } = await supabase
    .from('protocolos')
    .select('id, protocolo_id, tipo, entidad_id, estado, firma_fecha, firma_imagen_url')
    .eq('edp', EDP_OBJETIVO)
    .in('protocolo_id', TIPOS_HA)
    .order('protocolo_id')
    .order('entidad_id');

  if (error) {
    console.error(`❌ Error consultando protocolos: ${error.message}`);
    process.exit(1);
  }
  return data ?? [];
}
...
async function procesarProtocolo(id, idx, total) {
  ...
  // Si sigue sin camiones, regenerar produciría el mismo PDF vacío: no tocar
  // el archivo existente y dejar constancia para revisarlo a mano.
  if (camiones.length === 0) {
    console.log(`${prefix} ⚠ SKIP: sin camiones tras la consulta (camionId: ${camionId ?? 'ninguno'}) — ${label}`);
    return false;
  }

  if (DRY_RUN) {
    const guias = camiones.map(c => c.numeroGuia ?? '?').join(', ');
    console.log(`${prefix} 🔎 DRY-RUN — ${label} | ${camiones.length} camión(es) guía: ${guias} | firma: ${fmt(row.firma_fecha)}`);
    console.log(`        respaldaría en: backup-pdfs-edp7/${nombreRespaldo(row)}`);
    return true;
  }
  ...
  // Respaldo obligatorio: si no se puede guardar el PDF vigente, no se pisa.
  // Mejor dejarlo como está que perderlo.
  const respaldo = await respaldarPDFVigente(row, pdfPath);
  if (!respaldo) {
    console.log(`${prefix} ⚠ SKIP: no se pudo respaldar el PDF vigente, no se regenera — ${label}`);
    return false;
  }
  ...
  console.log(`${prefix} ✅ OK — ${label} | respaldo: ${nombreRespaldo(row)}`);
  return true;
}
...
async function main() {
  const modo = DRY_RUN ? ' (DRY-RUN — no escribe nada)' : '';
  console.log(`\n🔎 Consultando protocolos HA del EDP${EDP_OBJETIVO}...`);

  const protocolos = await obtenerProtocolosEDP7();
  const total      = protocolos.length;

  if (total === 0) {
    console.log('⚠ No se encontraron protocolos. Nada que hacer.\n');
    return;
  }

  console.log(`\nEncontrados ${total} protocolo(s):`);
  for (const p of protocolos) {
    const etiqueta = `${p.protocolo_id} ${NOMBRES_TIPO[p.tipo] ?? p.tipo} ${p.entidad_id}`;
    console.log(`   - ${etiqueta.padEnd(32)} estado: ${String(p.estado).padEnd(12)} firma: ${fmt(p.firma_fecha)}`);
  }

  if (total !== ESPERADOS) {
    console.log(`\n⚠ Se esperaban ${ESPERADOS} y la consulta devolvió ${total}. Revisa antes de continuar.`);
    if (!DRY_RUN) {
      console.log('   Abortando por seguridad. Ejecuta con --dry-run para inspeccionar.\n');
      process.exit(1);
    }
  }

  console.log(`\n🔄 Regenerando ${total} PDFs firmados HA (EDP${EDP_OBJETIVO})${modo}...\n`);

  let ok   = 0;
  let fail = 0;

  for (let i = 0; i < total; i++) {
    const exitoso = await procesarProtocolo(protocolos[i].id, i, total);
    if (exitoso) ok++;
    else         fail++;
  }

  console.log(`\n${'─'.repeat(40)}`);
  console.log(`✅ ${ok} exitosos  /  ❌ ${fail} omitidos o fallidos  (total: ${total})`);
  if (DRY_RUN) console.log('Sin cambios: fue un dry-run.');
