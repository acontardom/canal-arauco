...
const CARD_W   = { tramo: 60, caida: 35, atravieso: 45 };
...
// Divide array en grupos de `size`
...
// Parsea KM en formato "1.529,7" → 1529.7
...
export default function VistaCanal() {
  ...
  const [popup, setPopup]         = useState(null); // { tipo, id }
        ...
        <div style={s.filasCont}>
          {FILAS.map((fila, fi) => (
            <div key={fi} style={s.filaWrap}>
              {/* Etiqueta de fila */}
              <div style={s.filaLabel}>Fila {fi + 1}</div>
              {/* Scroll horizontal en móvil */}
              <div style={s.filaScroll}>
                <div style={s.fila}>
                  {/* Línea de canal */}
        ...
        <div style={s.overlay} onClick={() => setPopup(null)}>
          <div style={s.modal} onClick={e => e.stopPropagation()}>
            ...
            <h2 style={s.modalTitulo}>
              {NOMBRE_TIPO[popup.tipo]} {popup.id}
            </h2>
            ...
            {popupKm && (
              <div style={s.modalKmRow}>
                ...
                {largoMetros(popup.tipo, popup.id) !== null && (
                  <span style={s.modalLargo}>
                    {largoMetros(popup.tipo, popup.id).toLocaleString('es-CL')} m
                  </span>
                )}
              </div>
            )}
...
function Tarjeta({ tipo, id, avanceSet, onClick, onDotClick }) {
  ...
  return (
    ...
    >
      ...
      <span style={{ ...s.tarjetaLabel, fontSize: w <= 35 ? '9px' : '11px' }}>
        {cardLabel(tipo, id)}
      </span>
    </div>
  );
...
const s = {
  ...
  leyenda: { display: 'flex', alignItems: 'center', flexWrap: 'wrap', gap: '16px', marginBottom: '20px', padding: '10px 14px', background: '#0f2a4a', borderRadius: '8px', border: '1px solid #1e3a5f' },
  leyendaItem: { display: 'flex', alignItems: 'center', gap: '6px', color: '#ccd6f6', fontSize: '12px', fontWeight: 600 },
  leyendaPunto: { width: 10, height: 10, borderRadius: '50%', flexShrink: 0 },
  leyendaTipos: { display: 'flex', gap: '6px', marginLeft: 'auto' },
  ...
  filasCont: { display: 'flex', flexDirection: 'column', gap: '24px' },
  filaWrap: { display: 'flex', flexDirection: 'column', gap: '6px' },
  filaLabel: { color: '#4a5568', fontSize: '10px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.5px' },
  filaScroll: { overflowX: 'auto', overflowY: 'visible', paddingBottom: '4px' },
  ...
  lineaCanal: {
    position: 'absolute',
    height: '3px',
    background: 'linear-gradient(90deg, #1e4a7a 0%, #1e3a5f 100%)',
    left: 0,
    right: 0,
    top: '50%',
  ...
  // Popup/modal
  overlay: { position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', zIndex: 500, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '16px' },
  ...
  modalFlecha: { color: '#8892b0', fontSize: '16px', marginLeft: '2px' },
